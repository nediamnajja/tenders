...
# ==============================
# STEP 2+3 — Send cleaned notices to OpenAI and write results to enriched_tenders.

# Reads enricher/llm_step1_output.csv (produced by llm_step1_prefilter.py),
# calls OpenAI GPT-4o for each notice, and writes the 8 LLM fields directly
# into the llm_* columns of enriched_tenders.

# Also saves an audit CSV to enricher/llm_step2_output.csv so you can review
# what was written.
...
#     python enricher/llm_step2_enrich.py --model gpt4o_mini
#     python enricher/llm_step2_enrich.py --dry-run     # no DB writes
# """
...
# # Maps CSV field name → DB column name
# LLM_FIELD_MAP = {
...
# SECTOR_TAXONOMY = [
#     "energy", "water_sanitation", "transport", "digital",
#     "financial_management", "environment_social", "agriculture",
#     "health", "education", "governance", "extractives",
#     "trade_standards", "capacity_building", "urban_development",
# ]
...
# World Bank, and GCF-funded tenders. You extract structured intelligence from
# tender notice texts to help firms make fast, accurate bid/no-bid decisions.
...
# - For text fields: write in the same language as the notice (French notices give French output).
...
#   // Example: "Suriname Competitiveness and Sector Diversification Project — P166187 (IBRD Loan 8985-SR)"
...
#   // two_envelope = technical envelope opened first, financial held separately
#   // single_envelope = standard one-step sealed bid

#   "contract_duration_months": integer | null,
#   // Duration in months. Convert if needed: 8 weeks = 2 months, 1 year = 12 months.
#   // Use contract duration not overall project duration. null if not stated.

#   "eligibility_summary": string | null,
#   // A short bulleted list of the most important eligibility criteria.
#   // Format: each criterion on its own line starting with "• ".
#   // Use shortest possible wording — no full sentences, just key facts.
...
#     // Only if notice explicitly defines multiple lots, packages, clusters, or
#     // components that bidders apply to separately.
...
#     client = OpenAI()
#     model  = MODELS.get(model_key, MODELS["gpt4o"])
...
# def write_to_db(session, tender_db_id: int, data: dict) -> bool:
#     from models import EnrichedTender

#     enriched = (
#         session.query(EnrichedTender)
#         .filter(EnrichedTender.tender_id == tender_db_id)
#         .first()
#     )

#     if not enriched:
#         log.warning(
#             "  No enriched_tender for tenders.id=%s — run normalizer first",
#             tender_db_id
#         )
#         return False

#     for csv_field, db_field in LLM_FIELD_MAP.items():
#         raw_val = data.get(csv_field)
#         value   = coerce_value(csv_field, raw_val)
#         setattr(enriched, db_field, value)
...
# def run(
#     csv_path:  str = "enricher/llm_step1_output.csv",
#     limit:     int | None = None,
...
#         log.error("CSV not found: %s — run llm_step1_prefilter.py first", csv_path)
#         return
...
#     log.info("Loaded %d notices from %s", len(notices), csv_path)

#     # Filter
#     if target_id is not None:
#         notices = [n for n in notices if str(n["id"]) == str(target_id)]
#         if not notices:
#             log.error("No notice with id=%s in CSV", target_id)
#             return
...
#             cleaned   = notice.get("cleaned_text", "").strip()

#             if not cleaned:
#                 log.warning("  Skipping id=%s — empty cleaned_text", tender_id)
#                 continue

#             log.info("  [%d/%d] id=%s %s", i, len(notices), tender_id, title)

#             result = call_llm(cleaned, model_key=model_key)
#             total_tokens += result.get("tokens", 0)

#             if result["status"] != "ok":
#                 log.error("  API error id=%s: %s", tender_id, result.get("error"))
#                 api_errors += 1
#                 continue
...
#             if not dry_run:
#                 try:
#                     ok = write_to_db(session, int(tender_id), data)
#                     if ok:
#                         written += 1
#                         session.commit()
#                     else:
...
#                 "raw_notice_text": notice.get("raw_text", ""),
...
#     if audit_rows:
#         fieldnames = (
#             ["id", "portal", "title", "status", "tokens",
#              "raw_notice_text", "cleaned_text"] + FIELDS
#         )
...
#     # Cost estimate
#     cost_usd = (total_tokens / 1_000_000) * 0.30  # blended gpt-4omini rate
...
#     print(f"  No DB match (skip)     : {no_match}")
...
#     if no_match > 0:
#         print(
#             "  NOTE: Some tenders had no enriched_tender row.\n"
#             "  Run your normalizer on those tenders then re-run this script.\n"
#         )
...
#         description="Step 2: Send cleaned notices to OpenAI and write to DB."
#     )
#     parser.add_argument("--csv",     default="enricher/llm_step1_output.csv")
#     parser.add_argument("--limit",   type=int, default=None, metavar="N")
#     parser.add_argument("--id",      type=int, default=None, metavar="ID")
#     parser.add_argument("--model",   choices=["gpt4o", "gpt4o_mini"], default="gpt4o_mini")
#     parser.add_argument("--dry-run", action="store_true")
#     parser.add_argument("--audit",   default="enricher/llm_step2_output2.csv")
...
def run(
    ...
    try:
        ...
        for i, notice in enumerate(notices, 1):
            ...
            if result["status"] != "ok":
                ...
                api_errors += 1
                # Build audit row for failed call
                audit_rows.append({
                    "id":          tender_id,
    ...
    # Cost estimate (gpt-4o-mini blended rate ~$0.30/1M tokens)
