...
# # Pre-compute date span ranges once per call to avoid overlap with budget amounts.
# # Any amount whose match position falls inside a date string is rejected.
# def _date_spans(text: str) -> list[tuple[int, int]]:
#     """Return list of (start, end) char spans occupied by date strings."""
#     return [(m.start(), m.end()) for m in _DATE_PAT.finditer(text)]

# def _inside_date(pos: int, end: int, spans: list[tuple[int, int]]) -> bool:
#     """True if the match [pos:end] overlaps any known date span."""
#     return any(not (end <= ds or pos >= de) for ds, de in spans)

# def extract_budget_currency(text: str) -> tuple[Optional[float], Optional[str]]:
#     # Build date spans once so we can reject amounts that are really years in dates
...
#             )
#             amt_str_clean = re.sub(r"[\s,]", "", amt_str)
#             try:
#                 amt = float(amt_str_clean) * _parse_multiplier(mul_str)
#             except ValueError:
#                 continue

#             # ── Guard 1: reject bare year values (1900–2100) ─────────────────
#             # A 4-digit integer in that range is almost certainly a year leaking
#             # from a date string, a project number, or a reference code —
#             # never a real budget amount on its own.
#             raw_digits = amt_str_clean.replace(".", "")
#             if raw_digits.isdigit() and len(raw_digits) == 4:
#                 bare_val = int(raw_digits)
#                 if 1900 <= bare_val <= 2100:
#                     continue

#             # ── Guard 2: reject if match overlaps a date string ───────────────
#             # e.g. "2024" inside "15/06/2024" or "juin 2024"
#             if _inside_date(m.start(), m.end(), date_spans):
#                 continue

#             # ── Guard 3: currency must be within 60 chars of the amount ───────
#             # Prevents cross-sentence matches where a currency from one clause
#             # pairs with a number from a completely different clause.
#             ccy_pos = text.upper().find(ccy_raw.upper(), max(0, m.start() - 60))
#             if ccy_pos == -1 or abs(ccy_pos - m.start()) > 60:
#                 # Fallback: search forward too
#                 ccy_pos_fwd = text.upper().find(ccy_raw.upper(), m.start())
#                 if ccy_pos_fwd == -1 or abs(ccy_pos_fwd - m.start()) > 60:
#                     continue
...
FIELDS EXTRACTED:
    budget, currency, deadline_datetime, days_to_deadline,
    language, contact_email, contact_phone, procurement_group,
    description_clean (default fallback for UNDP and UNGM)
...
# ── Default descriptions for portals with no notice text ─────────────────────
DEFAULT_DESCRIPTIONS = {
    "undp": (
        "This tender is managed via UNDP Quantum, the supplier portal system. "
        "To submit a bid, subscribe following the instructions in the UNDP user guide. "
        "If not registered, create a supplier profile via the Supplier Registration link. "
        "If already registered, access the negotiation directly via the Supplier Portal "
        "and search using the reference number provided."
    ),
    "ungm": (
        "This opportunity is published through the United Nations Global Marketplace (UNGM), "
        "the common procurement portal of the United Nations system. "
        "To access the full tender details and submit a bid, you must be registered as a "
        "supplier on UNGM. Visit www.ungm.org to register or log in and search for this "
        "notice using the reference number provided."
    ),
}
...
_CURRENCY_ALIASES: dict[str, str] = {
    ...
    "FRANC CFA": "XOF", "FRANCS CFA": "XOF",
    "XAF": "XAF", "CFAF": "XAF", "FRANCS CFAF": "XAF",
    "TND": "TND", "DT": "TND", "DINAR": "TND", "DINARS": "TND", "DINAR TUNISIEN": "TND",
    "MAD": "MAD", "DH": "MAD", "DIRHAM": "MAD", "DIRHAMS": "MAD",
    "DZD": "DZD", "DA": "DZD", "DINAR ALGERIEN": "DZD", "DINAR ALGÉRIEN": "DZD",
    "NGN": "NGN", "NAIRA": "NGN",
    "KES": "KES", "KSH": "KES", "KENYA SHILLING": "KES",
...
def extract_budget_currency(text: str) -> tuple[Optional[float], Optional[str]]:
    ...
    for pat in _BUDGET_PATTERNS:
        for m in re.finditer(pat, text, re.IGNORECASE):
            gd = m.groupdict()
            amt_str = (gd.get("amt1") or gd.get("amt2") or gd.get("amt3") or gd.get("amt4") or gd.get("amt5") or "")
            mul_str = (gd.get("mul1") or gd.get("mul2") or gd.get("mul3") or gd.get("mul4") or gd.get("mul5"))
            ccy_raw = (gd.get("ccy1") or gd.get("ccy2") or gd.get("ccy3") or gd.get("ccy4") or gd.get("ccy5") or "")
            amt_str_clean = re.sub(r"[\s,]", "", amt_str)
            try:
                amt = float(amt_str_clean) * _parse_multiplier(mul_str)
            except ValueError:
...
_MONTHS_EN = (r"Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?"
              r"|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?")
_MONTHS_FR = (r"janvier|f[eé]vrier|mars|avril|mai|juin"
              r"|juillet|ao[uû]t|septembre|octobre|novembre|d[eé]cembre")
_MONTHS_AR = (r"يناير|فبراير|مارس|أبريل|مايو|يونيو"
              r"|يوليو|أغسطس|سبتمبر|أكتوبر|نوفمبر|ديسمبر")
_MONTHS_PAT = rf"(?:{_MONTHS_EN}|{_MONTHS_FR}|{_MONTHS_AR})"
_ORD  = r"(?:st|nd|rd|th|ème|eme|er|ère|ere)"
_WKDAY = (r"(?:Mon(?:day)?|Tue(?:sday)?|Wed(?:nesday)?|Thu(?:rsday)?"
          r"|Fri(?:day)?|Sat(?:urday)?|Sun(?:day)?"
          r"|[Ll]undi|[Mm]ardi|[Mm]ercredi|[Jj]eudi|[Vv]endredi|[Ss]amedi|[Dd]imanche)")
...
_MONTH_YEAR_PAT = re.compile(rf"\b({_MONTHS_PAT})\s+(\d{{4}})\b", re.IGNORECASE)

_TIME_PAT = re.compile(
    r"\b(\d{1,2})[h:](\d{2})\s*(?:hrs?|GMT|UTC|local\s+time|heure\s+locale|heure\s+de\s+\w+)?\b"
    r"|\b(\d{1,2})\s+heures?\s+(\d{2})\s*(?:minutes?)?\b"
    r"|\b(12\s*noon|midnight|minuit)\b"
    r"|\b(\d{1,2}:\d{2})\s*(AM|PM)\b",
    re.IGNORECASE,
)
...
def extract_deadline(text: str) -> Optional[datetime]:
    kw_positions: list[int] = [m.start() for m in _KW_PAT.finditer(text)]
    date_candidates: list[tuple[int, str]] = [(m.start(), m.group(0)) for m in _DATE_PAT.finditer(text)]
    ...
    for m in _MONTH_YEAR_PAT.finditer(text):
        ...
        try:
            dt = dateutil_parser.parse(f"1 {month_str} {year_str}", dayfirst=True)
            last_day = calendar.monthrange(dt.year, dt.month)[1]
            dt = dt.replace(day=last_day, hour=23, minute=59, second=59, tzinfo=timezone.utc)
        except (ValueError, OverflowError):
            continue
        return dt
...
_LANG_KEYWORDS: dict[str, list[str]] = {
    "English":    ["the","and","for","request","proposal","invitation","bid","procurement","tender","contract","services","goods","works","please","submit","deadline","closing","project","expressions","interest","applicants","eligible","shall","hereby"],
    "French":     ["le","la","les","de","du","pour","offre","appel","date","limite","marché","fourniture","travaux","soumission","dossier","entreprise","prestataire","montant","projet","contrat","consultants","candidature","financement","bénéficiaire"],
    "Arabic":     ["مناقصة","عطاء","طلب","عروض","مشروع","خدمات","توريد","أعمال","تقديم","الموعد","النهائي","العقد","المشتريات","المقاول"],
    "Portuguese": ["para","dos","das","projeto","proposta","concurso","fornecimento","obras","serviços","prazo","contrato","licitação","adjudicação","entidade","financiamento"],
    "Spanish":    ["para","los","las","proyecto","propuesta","licitación","suministro","obras","servicios","plazo","contrato","adquisición","entidad","financiamiento","oferta"],
}
...
def extract_language(text: str) -> str:
    ...
    for lang, keywords in _LANG_KEYWORDS.items():
        if lang == "Arabic":
            ...
            scores[lang] = sum(1 for kw in keywords if re.search(rf"\b{re.escape(kw)}\b", lower))
    best_lang  = max(scores, key=scores.get)
    best_score = scores[best_lang]
    if best_score < 3:
        return DEFAULT_LANGUAGE
...
_PHONE_RAW_RE = re.compile(r"(?<!\d)(\+?\d[\d\s\.\-\(\)\/]{6,20}\d)(?!\d)")
...
_PROC_GROUP_RULES: list[tuple[str, list[str]]] = [
    ("CONSULTING", [
        r"\bconsult(?:ing|ancy|ant)s?\b",r"\btechnical\s+assistance\b",r"\bTA\b",
        r"\bexperts?\b",r"\baudits?\b",r"\bsupervi(?:sion|sor)\b",r"\bdue\s+diligence\b",
        r"\bfeasibility\s+stud(?:y|ies)\b",r"\bmanagement\s+(?:support|services?)\b",
        r"\bcapacity\s+building\b",r"\btraining\s+(?:services?|program(?:me)?)\b",
        r"\bmonitoring\s+(?:and\s+)?evaluation\b",r"\bM&E\b",
        r"\bconsultants?\b",r"\bconsultance\b",r"\bprestations?\s+intellectuelles?\b",
        r"\b[eé]tudes?\b",r"\bassistance\s+technique\b",r"\bformation\b",r"\baudit\b",r"\bsupervision\b",
        r"\bاستشارات\b",r"\bخبراء\b",r"\bمساعدة\s+تقنية\b",
    ]),
    ("WORKS", [
        r"\bconstruct(?:ion|ing)\b",r"\brehabilitat(?:ion|ing)\b",r"\binfrastructure\b",
        r"\bcivil\s+works?\b",r"\brenovation\b",r"\binstallation\b",r"\bbuilding\b",
        r"\bbridge\b",r"\broad(?:s|way)?\b",r"\bdam\b",r"\birrigation\b",
        r"\bsanitation\b",r"\belectrification\b",
        r"\btravaux\b",r"\bréhabilitation\b",r"\bconstruction\b",r"\bgénie\s+civil\b",
        r"\bouvrage(?:s)?\b",r"\baménagement\b",r"\bassainissement\b",r"\bélectrification\b",
        r"\bأعمال\s+مدنية\b",r"\bإنشاء\b",r"\bتشييد\b",
    ]),
    ("NON-CONSULTING", [
        r"\bnon[\s\-]consult\w+\b",r"\boperational\s+services?\b",r"\bcleaning\s+services?\b",
        r"\bsecurity\s+services?\b",r"\btransport(?:ation)?\s+services?\b",
        r"\bmaintenance\s+services?\b",r"\bcatering\b",r"\bguard(?:ing)?\s+services?\b",
        r"\bwaste\s+management\b",
        r"\bservices?\s+non[\s\-]intellectuels?\b",r"\bentretien\b",r"\bnettoyage\b",
        r"\bsécurité\s+(?:privée|des\s+locaux)\b",r"\bgestion\s+des\s+déchets\b",
    ]),
    ("GOODS", [
        r"\bgoods?\b",r"\bequipment\b",r"\bsuppl(?:y|ies|ier)\b",r"\bvehicles?\b",
        r"\bcomputers?\b",r"\bmedical\s+(?:equipment|supplies|devices?)\b",
        r"\bprocurement\s+of\b",r"\bpurchase\s+of\b",r"\bdelivery\s+of\b",
        r"\bsupply\s+and\s+delivery\b",r"\bIT\s+equipment\b",
        r"\bfournitures?\b",r"\bmatériels?\b",
        r"\bacquisition\s+(?:de\s+)?(?:biens?|matériels?|équipements?)\b",r"\blivraison\b",
        r"\bتوريد\b",r"\bبضائع\b",r"\bمعدات\b",
    ]),
]
...
def normalize_structured_fields(raw: dict) -> dict:
    ...
    out["description_clean"] = raw.get("description_clean")
...
def extract_structured_fields(tender_dict: dict) -> StructuredExtraction:
    ...
    # ── Default description for portals with no notice text ───────────────────
    portal = tender_dict.get("source_portal", "")
    existing_desc = tender_dict.get("existing_description_clean") or tender_dict.get("description_clean")
    if not existing_desc and not full_text.strip() and portal in DEFAULT_DESCRIPTIONS:
        result.description_clean = DEFAULT_DESCRIPTIONS[portal]
        log.debug("  Applied default description for portal=%s", portal)
    else:
        result.description_clean = None  # don't overwrite if text exists
...
def merge_with_existing(existing: dict, extracted: dict) -> dict:
    ...
    for key in extracted:
        if key == "days_to_deadline":
            merged[key] = extracted[key]
            continue
        # description_clean: only fill if existing is empty
        if key == "description_clean":
            merged[key] = existing.get(key) or extracted.get(key)
            continue
        ev = existing.get(key)
        merged[key] = ev if (ev is not None and ev != "") else extracted.get(key)
    return merged
...
def process_tender(tender_dict: dict, dry_run: bool = False) -> dict:
    ...
    raw_dict = {
        ...
        "contact_phone":     extraction.contact_phone,
        "procurement_group": extraction.procurement_group,
        "description_clean": extraction.description_clean,  # ← default desc included
    }
    ...
    if not dry_run:
        try:
            from db import get_session
            with get_session() as session:
                update_enriched_tender(session, tender_dict["enriched_tender_id"], merged)
        except Exception as e:
            log.error("  DB write failed for enriched_tender_id=%s: %s",
                      tender_dict.get("enriched_tender_id"), e)
...
def run_rule_extraction(
        ...
        tender_dicts = [
            {
                ...
                "existing_contact_phone":            et.contact_phone,
                "existing_procurement_group":        et.procurement_group,
                "existing_description_clean":        et.description_clean,  # ← pass existing
            }
            for et, t in rows
        ]

    counters["total"] = len(tender_dicts)
    log.info("Found %d seeded rows pending rule extraction (portals: %s)",
             counters["total"], ", ".join(portals))
    ...
    for td in tender_dicts:
        log.info("Processing enriched_tender_id=%s (tender_id=%s)",
                 td["enriched_tender_id"], td["tender_id"])
        try:
            merged = process_tender(td, dry_run=dry_run)
            if dry_run:
            ...
            log.error("  Failed enriched_tender_id=%s: %s",
                      td.get("enriched_tender_id"), e, exc_info=True)
            counters["failed"] += 1

    log.info("Rule extraction done — total=%d  success=%d  failed=%d%s",
             counters["total"], counters["success"], counters["failed"],
             "  [DRY-RUN]" if dry_run else "")
...
_TEST_TENDERS = [
    {
        "enriched_tender_id": 1, "tender_id": 1001, "source_portal": "afdb",
        "title_clean": "Recrutement d'un consultant individuel — RCA",
        "notice_text": (
            "Les demandes d'éclaircissements doivent être adressées à la Coordination "
            "par courrier électronique au plus tard le vendredi 6 mars 2026 à 17h00.\n"
    ...
    {
        "enriched_tender_id": 2, "tender_id": 1002, "source_portal": "afdb",
        "title_clean": "General Procurement Notice — ISEDRMP Liberia",
        "notice_text": (
            "Loan: UA 13.83 Million equivalent to USD 18.30 Million.\n"
            "Bidding documents are expected to be available in March 2026.\n"
    ...
    # UNDP — no notice_text → should get default description
    {
        "enriched_tender_id": 8, "tender_id": 1008, "source_portal": "undp",
        "title_clean": "Consultancy Services — UNDP Peru",
        "notice_text": "",
        "existing_budget": None, "existing_currency": None,
        "existing_deadline_datetime": None, "existing_days_to_deadline": None,
        "existing_language": None, "existing_contact_email": None,
    ...
    # UNGM — no notice_text → should get default description
    {
        "enriched_tender_id": 9, "tender_id": 1009, "source_portal": "ungm",
        "title_clean": "Supply of Office Equipment — UNGM",
        "notice_text": "",
        "existing_budget": None, "existing_currency": None,
        "existing_deadline_datetime": None, "existing_days_to_deadline": None,
        "existing_language": None, "existing_contact_email": None,
...
def run_self_test() -> None:
    print("\n" + "=" * 82)
    print("  SELF-TEST — stage2b (anchor-window + default descriptions)")
    print("=" * 82)

    for td in _TEST_TENDERS:
        eid = td["enriched_tender_id"]
        print(f"\n{'─'*82}")
        print(f"  ID     : {eid}  |  portal: {td['source_portal']}")
        print(f"  Title  : {td['title_clean'][:78]}")
        print(f"{'─'*82}")
        ...
        dl = merged.get("deadline_datetime")
        print(f"  deadline_datetime   : {dl.strftime('%Y-%m-%d %H:%M UTC') if dl else 'None'}")
        print(f"  days_to_deadline    : {merged.get('days_to_deadline')}")
        print(f"  currency            : {merged.get('currency')}")
        print(f"  procurement_group   : {merged.get('procurement_group')}")
        print(f"  language            : {merged.get('language')}")
        print(f"  contact_email       : {merged.get('contact_email')}")

        desc = merged.get("description_clean")
        if desc:
            print(f"  description_clean   : {desc[:80]}…")
        else:
            print(f"  description_clean   : None")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit",    type=int, default=None, metavar="N")
    parser.add_argument("--portals",  nargs="+", default=DEFAULT_PORTALS, metavar="PORTAL")
    parser.add_argument("--test",     action="store_true", help="Run self-test (no DB).")
    args = parser.parse_args()
