...
4. For each tender:
   ...
   - If p_go >= 0.65 → GO, if p_go >= 0.80 → STRONG GO
   - Generates justification and writes to tender_scores
...
def should_exclude(tender) -> tuple[bool, str]:
    """
    Returns (True, reason) if tender should be excluded from scoring.
    Returns (False, "") if tender should be scored normally.
    """
    # Filter 1 — expired or imminent deadline (already handled by
    # get_unscored_enriched_tenders but double-checked here)
    ...
    # Filter 2 — Individual Consultant / Contractor procurement
...
# =============================================================================
# 8. RECOMMENDATION LABEL — updated thresholds
# =============================================================================
    ...
    with get_session() as session:
        ...
        for tender in tenders:
            try:
                ...
                if recommendation:
                    justification = generate_justification(contributions, p_go, recommendation)
                    save_tender_score(
                        session            = session,
                        enriched_tender_id = tender.id,
                        p_go               = p_go,
