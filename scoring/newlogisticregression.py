...
#    - Writes a tender_scores row for EVERY scored tender (no threshold gate)
#    - If p_go >= 0.70 → GO, if p_go >= 0.80 → STRONG GO (label only)
#    - Generates justification for GO/STRONG GO tenders
...
#     "Cameroon": 2, "Cameroun": 2, "Ghana": 2, "Nigeria": 2, "Kenya": 2,
#     "South Africa": 2, "Angola": 2, "Mozambique": 2, "Ethiopia": 2,
#     "Tanzania": 2, "United Republic of Tanzania": 2, "Uganda": 2,
#     "Rwanda": 2, "Zambia": 2, "Zimbabwe": 2, "Gabon": 2,
#     "Benin": 2, "Bénin": 2, "Togo": 2, "Burkina Faso": 2, "Mali": 2,
#     "Guinea": 2, "Guinée": 2, "Congo": 2,
#     "Democratic Republic Of The Congo": 2, "Mauritius": 2,
#     "Namibia": 2, "Madagascar": 2, "Liberia": 2, "Sierra Leone": 2,
...
#     "Guinea Bissau": 3, "Guinée Bissau": 3, "Lesotho": 3,
#     "Swaziland": 3, "Seychelles": 3, "Malawi": 3, "Malawai": 3,
#     "France": 4, "Germany": 4, "Italy": 4, "Denmark": 4, "Cyprus": 4,
#     "Albania": 4, "Kosovo": 4, "Serbia": 4, "Montenegro": 4,
#     "Bosnia And Herzegovina": 4, "Republic Of North Macedonia": 4,
#     "Romania": 4, "Republic of Moldova": 4, "Ukraine": 4,
#     "Georgia": 4, "Turkey": 4, "Turkiye": 4, "Armenia": 4,
...
#     "St. Lucia": 5, "St Maarten": 5, "Plurinational State of Bolivia": 5,
...
#     "Palestinian Territory": 6, "Papua New Guinea": 6, "Fiji": 6,
#     "Tuvalu": 6, "Kiribati": 6, "Marshall Islands": 6, "Pacific 1": 6,
# }
...
#     "Enterprise IT & Systems Implementation", "Business Strategy & Performance",
...
# def should_exclude(tender) -> tuple[bool, str]:
#     if tender.days_to_deadline is not None and tender.days_to_deadline < 2:
#         return True, f"deadline too close or passed ({tender.days_to_deadline} days)"
...
#     p_go = 1 / (1 + math.exp(-Z))
#     p_go = round(p_go * 0.80, 4)  # rescale to max 80%
#     return p_go, contributions
   

# # =============================================================================
# # 7. JUSTIFICATION GENERATOR
# # =============================================================================
...
#     "agency_FAO": "FAO", "agency_ILO": "ILO", "agency_UNICEF": "UNICEF",
#     "agency_IOM": "IOM", "agency_UNOPS": "UNOPS", "agency_UNIDO": "UNIDO",
...
# def generate_justification(contributions: dict, p_go: float, recommendation: str) -> str:
...
#     lines = [f"{'='*52}", f"  {recommendation}  —  P(GO) = {p_go:.1%}", f"{'='*52}"]
...
#     if p_go >= THRESHOLD_STRONG_GO: return "STRONG GO"
#     if p_go >= THRESHOLD_GO:        return "GO"
#     return None

# # =============================================================================
# # 9. MAIN SCORING ENGINE
# # =============================================================================

# def run_scoring_engine():
#     logger.info("=" * 55)
#     logger.info("  KPMG SCORING ENGINE — Starting")
#     logger.info(f"  GO threshold        : {THRESHOLD_GO:.0%}")
#     logger.info(f"  STRONG GO threshold : {THRESHOLD_STRONG_GO:.0%}")
#     logger.info("=" * 55)
...
#             try:
#                 # ── Hard filter ───────────────────────────────
#                 skip, reason = should_exclude(tender)
#                 if skip:
#                     logger.debug(f"Excluded tender {tender.id}: {reason}")
#                     excluded += 1
#                     continue
...
#                     "Z": round(baseline + sum(contributions.values()), 4),
#                     "p_go": p_go,
#                 })
...
#                 # ── Save tender_scores row for EVERY scored tender ────────────
#                 # recommendation is None for below-threshold tenders
#                 # justification is only generated for GO / STRONG GO
#                 recommendation = get_recommendation(p_go)
#                 justification  = generate_justification(contributions, p_go, recommendation) if recommendation else None
...
#         logger.info(f"  Total processed    : {len(tenders)}")
#         logger.info(f"  Excluded (filtered): {excluded}")
#         logger.info(f"  Scored             : {scored}")
#         logger.info(f"  STRONG GO (≥{THRESHOLD_STRONG_GO:.0%})  : {strong_go}")
#         logger.info(f"  GO ({THRESHOLD_GO:.0%}-{THRESHOLD_STRONG_GO:.0%})       : {go_count - strong_go}")
...
def load_latest_weights(session) -> tuple[dict, float, int]:
    ...
    # ── Clip weights — no single feature can exceed ±2.0 ─────────────────────
    # Prevents any one feature from dominating the score regardless of SGD drift
    weights = {k: max(-3.0, min(2.0, v)) for k, v in weights.items()}
...
def compute_score(features: dict, weights: dict, baseline: float) -> tuple[float, dict]:
    ...
    for feature, x in features.items():
        if x == 1 and feature in weights:
            ...
            contributions[feature] = round(contribution, 4)
    p_go = 1 / (1 + math.exp(-Z))
    p_go = round(p_go * 0.80, 4)  # rescale — max 80%
    return p_go, contributions
